(function () {
  ...
  // ── Next Gathering hydration ──────────────────────────────────────────
  // The next session lives in the calendar_events table (DM-scheduled), so
  // any page with a [data-next-gathering] block renders it client-side
  // instead of baking it in at build time.

  function gatheringDaysLabel(iso) {
  ...
  function renderGathering(root, gathering) {
  ...
  async function initNextGathering() {
