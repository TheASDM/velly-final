---
title: Player Characters
description: The seven adventurers of the Vallombrosa campaign — a wizard-turned-warlock, an orc Fog Warden, a fog-child cleric, a forest gnome arcane trickster, a satyr masquerade bard, a tiefling pirate, and an elf ranger.
published: true
date: 2026-05-23T00:00:00.000Z
tags: characters, pcs, index, navigation
editor: markdown
dateCreated: 2026-05-23T00:00:00.000Z
---

<section class="vos-section-header">
  <p class="vos-section-kicker">Cast</p>
  <h1 class="vos-section-title">Player Characters</h1>
</section>

<div class="vos-row-chip-list">
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/caravel-asteri/">
    <span>
      <span class="vos-row-chip-title">Caravel &quot;Car&quot; Asteri</span>
      <span class="vos-row-chip-meta">Satyr Masquerade Bard. Venturia-born, finished with secrets, tied to the Covenant Archive family they walked away from.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">Car</span>
  </a>
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/kryton-novelli/">
    <span>
      <span class="vos-row-chip-title">Kryton Novelli</span>
      <span class="vos-row-chip-meta">Human Warlock, former Wizard. A VAVA faculty mage with a gambling-table pact he intends to escape.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">KN</span>
  </a>
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/lotan/">
    <span>
      <span class="vos-row-chip-title">Lotan</span>
      <span class="vos-row-chip-meta">Tiefling Fighter/Warlock. Stranded ashore after looking for a Kraken and managing to offend it.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">Lo</span>
  </a>
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/noname/">
    <span>
      <span class="vos-row-chip-title">Noname</span>
      <span class="vos-row-chip-meta">Orc Barbarian Fog Warden. Raised at Liminal House, now hunting the memory of the night her fiance vanished.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">FW</span>
  </a>
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/orabella/">
    <span>
      <span class="vos-row-chip-title">Orabella</span>
      <span class="vos-row-chip-meta">Human Cleric of Art. A fog child raised by Liminal House, haunted by vivid dreams she cannot explain.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">Or</span>
  </a>
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/roxanya/">
    <span>
      <span class="vos-row-chip-title">Roxanya &quot;Roxy&quot;</span>
      <span class="vos-row-chip-meta">Forest Gnome Arcane Trickster. Brownie-raised, Academy-enrolled on a lie, pact-bound to Garl Glittergold.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">Rx</span>
  </a>
  <a class="vos-row-chip" href="/en/Venturia/Characters/PCs/valentro/">
    <span>
      <span class="vos-row-chip-title">Valentro</span>
      <span class="vos-row-chip-meta">Elf Ranger. Keeps the Tiered Gardens while learning swordfighter reflexes he cannot remember earning.</span>
    </span>
    <span class="vos-row-chip-badge" aria-hidden="true">Va</span>
  </a>
</div>

---

*The panels above are the cast in play. Everything else in this section is listed below.*
