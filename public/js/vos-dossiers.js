(()=>{var D=[{label:"Owes",key:"P1 - who you owe"},{label:"3am person",key:"P1 - your 3am person"},{label:"Mask",key:"P1 - your mask"},{label:"Tell",key:"P1 - your tell"},{label:"Vice",key:"P1 - your vice"},{label:"Wants",key:"P1 - what you want"}],F="#D0AE5E";function K(e){let s=window.VOS_PWA;return s&&s.authHeaders?s.authHeaders(e||{}):e||{}}async function P(e,s){let t=await fetch(e,{cache:"no-store",...s||{}});if(!t.ok){let n=await t.json().catch(()=>({})),l=new Error(n.error||`HTTP ${t.status}`);throw l.status=t.status,l}return t.json()}function R(e){let s=Object.values(e)[0];return s?s.vitals.map(t=>({label:t.group,fields:t.fields.map(n=>({key:n.key,label:n.label}))})):[]}function _(e){let s={};return e.part1.forEach(t=>{s[t.key]=t.prompt}),Object.values(e.characters).forEach(t=>{t.part2.forEach(n=>{s[n.key]=n.prompt})}),s[e.codaKey]=e.codaPrompt,s}function O(e){let s={};return e.vitals.forEach(t=>{t.fields.forEach(n=>{n.onFile&&n.value&&(s[n.key]=n.value)})}),s}function B(e,s,t){let n=O(s),l=[...e.part1.map(a=>a.key),...s.part2.map(a=>a.key),...s.vitals.flatMap(a=>a.fields.map(i=>i.key)),e.codaKey],r=0;return l.forEach(a=>{let i=(t[a]||"").trim();(i||!i&&n[a])&&(r+=1)}),{answered:r,total:l.length}}async function S(){let[e,s,t]=await Promise.all([P("/data/questionnaire.json"),P("/data/players.json"),P("/api/questionnaire/all",{headers:K()})]),n={};(t.records||[]).forEach(a=>{n[a.playerName]=a});let l={};s.forEach(a=>{l[a.name]=a});let r=Object.keys(e.characters).map(a=>{let i=e.characters[a],d=n[i.player]||{},u=d.answers||{},m=l[i.player]||{},{answered:w,total:H}=B(e,i,u);return{id:a,player:i.player,name:i.name,short:m.display||i.name.split(" ")[0].replace(/[“”"]/g,""),role:i.role||"",color:m.color||F,status:d.status||"draft",submitted:d.submitted_at||null,updated:d.updated_at||null,answers:u,onFile:O(i),p2:i.part2.map(V=>V.key),answered:w,total:H}});return{p1:e.part1.map(a=>a.key),codaKey:e.codaKey,prompts:_(e),vitalsGroups:R(e.characters),characters:r}}function o(e){return String(e??"").replace(/[&<>"]/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[s])}function v(e){return String(e||"").replace(/^P1 - /,"").replace(/^P2 - /,"").replace(/^vitals - /,"")}function p(e,s){return((e.answers||{})[s]||"").trim()}function T(e){if(!e)return"\u2014";let s=new Date(e);return Number.isNaN(s.getTime())?"\u2014":s.toISOString().slice(0,10)}function g(e,s){return s?Math.round(e/s*100):0}function L(e,s){let t=s.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return o(e).replace(new RegExp("("+t+")","ig"),"<mark>$1</mark>")}var W={2:"two",3:"three",4:"four",5:"five",6:"six",7:"seven",8:"eight"},G=["P1 - who you owe","P1 - your 3am person","P1 - an unspoken fear","P1 - what you want"];function Y(e){let s=G.map(t=>{let n=p(e,t);return`<dt>${o(v(t))}</dt><dd${n?"":' class="none"'}>${n?o(n):"\u2014"}</dd>`}).join("");return`<button class="card" data-char="${o(e.id)}" style="--c:${o(e.color)}">
    <h3>${o(e.name)}</h3>
  ...
  </button>`}function N(e){let s=e.characters,t=s.filter(r=>r.status==="submitted").length,n=s.reduce((r,a)=>r+a.answered,0),l=s.reduce((r,a)=>r+a.total,0);return`
  ...
  <div class="ov">${s.map(Y).join("")}</div>`}function A(e,s,t,n,l){let r=p(s,t);if(!r&&!l)return"";let a=e.prompts[t]||"";return`<div class="qa${r?"":" empty"}">
    <div class="label">${o(n)}</div>
    <div class="q">${o(v(t))}</div>
    ${a?`<div class="prompt">${o(a)}</div>`:""}
    <div class="a${r?"":" none"}">${r?o(r):"No answer yet"}</div>
  </div>`}function z(e,s,t){let n=p(e,s.key),l=(e.onFile[s.key]||"").trim();if(!n&&!l&&!t)return"";let r="\u2014",a="",i=!0;return n&&l&&n!==l?(r=n,a=`changed from on file: ${l}`,i=!1):n?(r=n,i=!1):l&&(r=l,a="on file, unchanged",i=!1),`<div class="vrow">
    <dt>${o(s.label)}</dt>
    <dd${i?' class="none"':""}>${o(r)}${a?`<span class="chg">${o(a)}</span>`:""}</dd>
      ...
      <h4>${o(l.label)}</h4>
      <dl class="vgrid">${r}</dl>
    </div>`:""}).join("")||'<div class="empty-state">Nothing filled in yet.</div>'}function C(e,s,t){let n=e.p1.filter(i=>p(s,i)).length,l=s.p2.filter(i=>p(s,i)).length,r=p(s,e.codaKey),a=D.map(i=>{let d=p(s,i.key);return`<div class="cell">
  ...
  <section class="sec" id="s-p1">
    ...
    ${e.p1.map(i=>A(e,s,i,"Part One",t)).join("")||'<div class="empty-state">No answers here.</div>'}
  </section>
  ...
  <section class="sec" id="s-p2">
    ...
    ${s.p2.map(i=>A(e,s,i,"Part Two",t)).join("")||'<div class="empty-state">No answers here.</div>'}
  </section>
  ...
  </section>`:""}`}function j(e){let s=e.p1.map(t=>({key:t,group:"Part One"}));return e.vitalsGroups.forEach(t=>{t.fields.forEach(n=>{s.push({key:n.key,group:"Vitals \xB7 "+t.label})})}),s}function M(e,s,t){let n=j(e),l=[...new Set(n.map(u=>u.group))],r=e.prompts[s]||"",a=e.characters.filter(u=>p(u,s)).length,i=l.map(u=>{let m=n.filter(w=>w.group===u).map(w=>`<option value="${o(w.key)}"${w.key===s?" selected":""}>${o(v(w.key))}</option>`).join("");return`<optgroup label="${o(u)}">${m}</optgroup>`}).join(""),d=e.characters.map(u=>{let m=p(u,s);return!m&&!t?"":`<div class="voice" style="--c:${o(u.color)}">
      <div class="who">${o(u.short)}</div>
  ...
  <div class="voices">${d||'<div class="empty-state">Nobody answered this one.</div>'}</div>`}function I(e,s){let t=s.trim().toLowerCase(),n=[];e.characters.forEach(a=>{Object.keys(a.answers||{}).forEach(i=>{let d=(a.answers[i]||"").trim();if(!d)return;`${d} ${v(i)} ${e.prompts[i]||""}`.toLowerCase().includes(t)&&n.push({character:a,key:i,value:d})})});let l=new Set(n.map(a=>a.character.id)).size,r=n.length?e.characters.map(a=>{let i=n.filter(d=>d.character.id===a.id);return i.length?`<section class="sec">
        <div class="sec-head">
