(()=>{function Le(t){let e=window.VOS_PWA;return e&&e.authHeaders?e.authHeaders(t||{}):t||{}}async function He(t,e){let n=await fetch(t,{cache:"no-store",...e||{},headers:Le({"Content-Type":"application/json",...(e||{}).headers||{}})}),s=await n.json().catch(()=>({}));if(!n.ok){let c=new Error(s.error||`HTTP ${n.status}`);throw c.status=n.status,c.code=s.error_code,c}return s}function P(t,e){let n=e?{...t,playerName:e}:t;return He("/api/play/op",{method:"POST",body:JSON.stringify(n)})}function ne(t,e,n){if(!t)return t;let s=JSON.parse(JSON.stringify(t)),c=n&&n.maxHp;switch(e.op){case"damage":{let i=Math.min(s.hp.temp||0,e.amount);return s.hp.temp-=i,s.hp.current!=null&&(s.hp.current=Math.max(0,s.hp.current-(e.amount-i))),s.hp.current===0?null:s}case"heal":{s.hp.current==null&&(s.hp.current=0);let i=s.hp.current+e.amount;return s.hp.current=c!=null?Math.min(i,c):i,s}case"spendSlot":{let i=String(e.level),v=((n||{}).slots||{})[i],C=(s.slots[i]||0)+1;return v!=null&&C>v?null:(s.slots[i]=C,s)}case"restoreSlot":{let i=String(e.level);return s.slots[i]=Math.max(0,(s.slots[i]||0)-1),s}case"useCharge":{let i=(s.uses[e.feature]||0)+1;return e.max!=null&&i>e.max?null:(s.uses[e.feature]=i,s)}case"restoreCharge":return s.uses[e.feature]=Math.max(0,(s.uses[e.feature]||0)-1),s;default:return null}}function oe(t,e){switch(t.op){case"damage":return{op:"setHp",value:e.hp.current,_restore:e};case"heal":return{op:"setHp",value:e.hp.current};case"setHp":return{op:"setHp",value:e.hp.current};case"setTempHp":return{op:"setTempHp",value:e.hp.temp,keepHigher:!1};case"spendSlot":return{op:"restoreSlot",level:t.level};case"restoreSlot":return{op:"spendSlot",level:t.level};case"useCharge":return{op:"restoreCharge",feature:t.feature};case"restoreCharge":return{op:"useCharge",feature:t.feature};case"addCondition":return{op:"removeCondition",condition:t.condition};case"removeCondition":return{op:"addCondition",condition:t.condition};case"adjustExhaustion":return{op:"adjustExhaustion",delta:-t.delta};case"setExhaustion":return{op:"setExhaustion",value:e.exhaustion};default:return null}}var O=new Map;async function U(t){if(O.has(t))return O.get(t);let e=fetch(t,{cache:"default"}).then(n=>{if(!n.ok)throw new Error(`HTTP ${n.status}`);return n.json()}).catch(n=>{throw O.delete(t),n});return O.set(t,e),e}function W(t){return U(t)}function ie(){return U("/data/play/conditions.json")}var re={bard:"bard",cleric:"cleric",ranger:"ranger",warlock:"warlock",wizard:"wizard",rogue:"wizard",fighter:"wizard"};function le(t){let e=t&&t.classes||[];for(let n of e){let s=String(n.identifier||n.name||"").toLowerCase();if(re[s])return re[s]}return null}async function ce(t){if(!t)return null;try{return await U(`/data/play/spells-${t}.json`)}catch{return null}}function ue(t,e){if(!t||!Array.isArray(t.prepared)||!e)return null;let n=t.prepared[e-1];return typeof n=="number"?n:null}function pe(){return W("/data/play/masquerade.json")}function de(){return W("/data/play/forms.json")}function me(t,e){if(!t||!e)return[];let n=new Set((t.features||[]).map(s=>(s.name||"").toLowerCase()).filter(s=>s.startsWith("maschera ")).map(s=>s.split(" ")[1]));return Object.values(e.masks).filter(s=>n.has(s.key)).sort((s,c)=>s.name.localeCompare(c.name))}function z(t){return t&&t.level>=6?3:1}function J(t,e,n){return!t||!e||!e.type?[]:(t[e.type]||[]).filter(s=>s.crValue<=n)}function R(t){let e=Math.max(0,Math.floor(t/1e3));return`${Math.floor(e/60)}:${String(e%60).padStart(2,"0")}`}function fe(t,e){let n=[],s=e&&e.spellcasting&&e.spellcasting.dc,c=e&&(e.abilities||[]).find(i=>i.key==="int")||null;return c&&t.abilities&&t.abilities.int!==c.score&&n.push({label:"Intelligence",value:`${c.score} (${c.mod>=0?"+":""}${c.mod})`,why:"Yours \u2014 only Intelligence, memories and alignment stay."}),s&&n.push({label:"Save DC",value:String(s),why:"Any DC in this creature's abilities uses your spell save DC."}),n.push({label:"Bardic Inspiration",value:"kept",why:"Retained in any form. You cannot cast unless the form can."}),n}var Ae=[1,2,3,4,5,6,7,8,10,12,15,20],qe=12;function ye(t){let e=t.root,n=t.onState,s=t.playerName||null,c=t.model||null,i=t.state,v=t.limits,C=!1,w=null,E=null,A=null;function ve(){if(navigator.vibrate)try{navigator.vibrate(qe)}catch{}}async function h(o,{undoable:a=!0}={}){if(C)return;C=!0;let r=i;ve();let l=ne(i,o,v);l&&(i=l,n(i,{optimistic:!0}));try{let d=await P(o,s);i=d.state,v=d.limits||v,n(i,{note:d.note}),a&&be(o,r,d.note)}catch(d){i=r,n(i,{error:d.message}),H(d.message,{tone:"error"})}finally{C=!1}}function be(o,a,r){let l=oe(o,a);H(r||"Done",{action:l?{label:"Undo",run:()=>h(l,{undoable:!1})}:null})}let g=null,G=null;function H(o,{tone:a="",action:r=null}={}){if(g||(g=document.createElement("div"),g.className="vos-play-toast",g.setAttribute("role","status"),document.body.appendChild(g)),g.className=`vos-play-toast is-on${a?` is-${a}`:""}`,g.innerHTML='<span class="vos-play-toast-text"></span>',g.querySelector(".vos-play-toast-text").textContent=o,r){let l=document.createElement("button");l.type="button",l.className="vos-play-toast-action",l.textContent=r.label,l.addEventListener("click",()=>{I(),r.run()}),g.appendChild(l)}clearTimeout(G),G=setTimeout(I,r?6e3:3e3)}function I(){g&&g.classList.remove("is-on")}let S=null;function L(o,a,r){b(),S=document.createElement("div"),S.className="vos-play-sheet",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),S.addEventListener("click",d=>{d.target.closest("[data-close]")&&b()}),r&&r(S);let l=S.querySelector("button:not([data-close])");l&&l.focus()}function b(){S&&(S.remove(),S=null)}function Q(){let o=v&&v.maxHp||0,a=i.hp.current!=null?i.hp.current:o,r=Ae.map(l=>`<button type="button" class="vos-play-num" data-amount="${l}">${l}</button>`).join("");L("Hit points",`
    ...
    `,r=>{r.querySelector(".vos-play-custom").addEventListener("submit",l=>{l.preventDefault();let d=Number(l.target.querySelector("input").value)||0;b(),h({op:"spendHitDie",healed:d})}),r.querySelector("[data-nothing]").addEventListener("click",()=>{b(),h({op:"spendHitDie"})})})}function ge(){L("Rest",`
    ...
    `,o=>{o.querySelectorAll("[data-rest]").forEach(a=>{a.addEventListener("click",()=>{let r=a.dataset.rest;r==="longRest"&&!window.confirm("Take a long rest?")||(b(),h({op:r}))})})})}let ke=["blinded","charmed","deafened","frightened","grappled","incapacitated","invisible","paralyzed","petrified","poisoned","prone","restrained","stunned","unconscious"];async function X(){let o=new Set(i.conditions||[]),a={};try{a=await ie()}catch{}let r=ke.map(u=>{let y=a[u]||{};return`<div class="vos-play-cond-row${o.has(u)?" is-on":""}">
      ...
      </div>`}).join(""),l=i.concentration,d=`<div class="vos-play-conc">
    ...
    `,d=>{d.querySelectorAll("[data-form]").forEach(u=>{u.addEventListener("click",()=>{let y=r[Number(u.dataset.form)];b(),h({op:"assumeForm",creature:y.name,source:y.source,cr:y.cr,hp:y.hp})})})})}async function Ce(){if(!i.form||!await B())return"";let o=E.masks[i.mask?i.mask.key:""],r=(A&&o&&A[o.type]||[]).find(u=>u.name===i.form.creature);if(!r)return"";let l=fe(r,c).map(u=>`
      ...
      <article class="vos-play-formblock">
        ...
        <div class="vos-play-form-vitals">
          ...
          <span><b>${m(i.form.hp)}</b>/${m(i.form.maxHp)} HP</span>
          <span><b>${m(r.speed)}</b>Speed</span>
        </div>
        <div class="vos-play-overrides">${l}</div>
        ${d(r.traits,"Traits")}
        ${d(r.actions,"Actions")}
      </article>`}function te(o){return(c&&c.activatable||[]).find(a=>a.id===o)||null}function ae(o){return!!(i.active||{})[o]}function se(o){let a=Number((i.uses||{})[o.id]||0);return Math.max(0,o.uses.max-a)}function Ee(o){let a=te(o);if(a){if(ae(o)){_(o);return}if(!se(a)){H(`No uses of ${a.name} left \u2014 a rest brings them back.`,{tone:"warn"});return}h({op:"activateFeature",feature:a.id,name:a.name,max:a.uses.max})}}function _(o){let a=te(o);if(!a)return;let r=ae(o),l=se(a);L(a.name,`
      <p class="vos-play-feature-state">${r?"Active now.":`${l} of ${a.uses.max} uses left${a.uses.recovery?`, back on a ${m(a.uses.recovery)}`:""}${a.activation?` \xB7 ${m(a.activation)}`:""}.`}</p>
      <ul class="vos-play-feature-grants">${a.grants.map(d=>`<li>${m(d)}</li>`).join("")}</ul>
      ${r?'<button type="button" class="vos-play-btn is-danger" data-feature-act="end">End it</button>':`<button type="button" class="vos-play-btn is-primary" data-feature-act="start"${l?"":" disabled"}>${l?`Use ${m(a.name)}`:"No uses left"}</button>`}
    `,d=>{let u=d.querySelector("[data-feature-act]");u&&u.addEventListener("click",()=>{b(),h(u.dataset.featureAct==="end"?{op:"endFeature",feature:a.id}:{op:"activateFeature",feature:a.id,name:a.name,max:a.uses.max})})})}function T(o){let a=o.target.closest("[data-play]");if(!(!a||!e.contains(a))&&!(o.type==="keydown"&&!["Enter"," "].includes(o.key)))switch(o.type==="keydown"&&o.preventDefault(),a.dataset.play){case"hp":Q();break;case"hitdice":$e();break;case"slot":{let r=Number(a.dataset.level),l=a.dataset.spent==="1";h({op:l?"restoreSlot":"spendSlot",level:r});break}case"pact":{if(a.dataset.spent==="1")return;h({op:"spendPactSlot"});break}case"charge":{let r=a.dataset.feature;if(!r)return;h({op:"useCharge",feature:r,max:Number(a.dataset.max)||void 0});break}case"charge-pip":{let r=a.dataset.feature;if(!r)return;h(a.dataset.spent==="1"?{op:"restoreCharge",feature:r}:{op:"useCharge",feature:r,max:Number(a.dataset.max)||void 0});break}case"feature":_(a.dataset.feature);break;case"end-feature":h({op:"endFeature",feature:a.dataset.feature});break;case"exhaustion":{let r=Number(a.dataset.value);h({op:"setExhaustion",value:r===i.exhaustion?r-1:r});break}case"conditions":case"condition":X();break;default:break}}return e.addEventListener("click",T),e.addEventListener("keydown",T),{apply:h,openHpPad:Q,openRests:ge,openConditions:X,openPrepare:we,openMasks:ee,openForms:xe,openFeature:_,activateFeature:Ee,formStatblockHtml:Ce,setState(o,a){i=o,a&&(v=a)},destroy(){e.removeEventListener("click",T),e.removeEventListener("keydown",T),b(),I()}}}var Ne=12e3,N=document.getElementById("vos-party-root"),V=document.getElementById("vos-party-status"),F=[],Y=null,q=!1;function k(t){return String(t??"").replace(/[&<>"]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[e])}function Me(t=6e3){return new Promise(e=>{let n=Date.now();(function s(){if(window.VOS_PWA)return e(window.VOS_PWA);if(Date.now()-n>t)return e(null);setTimeout(s,80)})()})}function De(t){let e=window.VOS_PWA;return e&&e.authHeaders?e.authHeaders(t||{}):t||{}}async function Te(){let t=await fetch("/api/play/party",{cache:"no-store",headers:De()}),e=await t.json().catch(()=>({}));if(!t.ok){let n=new Error(e.error||`HTTP ${t.status}`);throw n.status=t.status,n}return e.party||[]}function je(t,e){if(!e||t==null)return"";let n=t/e;return t===0?"is-down":n<=.25?"is-bloodied":n<=.5?"is-hurt":""}function Pe(t){let e=t.limits&&t.limits.slots||{},n=t.state.slots||{},s=Object.keys(e).sort();return s.length?s.map(c=>{let i=e[c],v=Math.max(0,i-(n[c]||0));return`<span class="vos-party-slot${v?"":" is-empty"}">
      <i>${k(c)}</i>${v}<b>/${i}</b></span>`}).join(""):""}function Oe(t){let e=t.state,n=t.limits&&t.limits.maxHp||0,s=e.hp.current!=null?e.hp.current:n,c=n?Math.max(0,Math.min(100,s/n*100)):0,i=(e.conditions||[]).includes("dying"),v=(e.conditions||[]).filter(w=>w!=="dying"),C=Math.max(0,(t.limits&&t.limits.hitDice||0)-(e.hitDiceSpent||0));return t.hasStatblock?`<article class="vos-party-card ${je(s,n)}${i?" is-dying":""}"
    ...
    </article>`}function K(){N.innerHTML=F.map(Oe).join("")}function M(t,e=""){V&&(V.textContent=t,V.className=`vos-party-status${e?` is-${e}`:""}`)}function Re(t){let e=F.find(n=>n.playerName===t);return e?ye({root:N,playerName:t,state:e.state,limits:e.limits,onState(n){e.state=n,K()}}):null}async function Fe(t,e){let n=e==="heal"?"Heal":"Damage",s=Number(window.prompt(`${n} ${t.character} by how much?`,"5"));if(!(!Number.isFinite(s)||s<=0))try{let c=await P({op:e==="heal"?"heal":"damage",amount:s},t.playerName);t.state=c.state,t.limits=c.limits||t.limits,K()}catch(c){M(c.message,"error")}}function Ie(t){let e=t.target.closest("[data-act]");if(!e)return;let n=e.closest("[data-player]");if(!n)return;let s=F.find(v=>v.playerName===n.dataset.player);if(!s)return;let c=e.dataset.act;if(c==="damage"||c==="heal"){Fe(s,c);return}let i=Re(s.playerName);i&&(c==="hp"&&i.openHpPad(),c==="conditions"&&i.openConditions())}async function D({quiet:t=!1}={}){t||M("Refreshing\u2026");try{F=await Te(),K(),M(`Updated ${new Date().toLocaleTimeString()}`)}catch(e){if(e.status===401||e.status===403){M("DM only.","error"),he();return}M(`Could not refresh \u2014 ${e.message}`,"error")}}function Be(){he(),Y=setInterval(()=>{!q&&!document.hidden&&D({quiet:!0})},Ne)}function he(){clearInterval(Y),Y=null}document.addEventListener("visibilitychange",()=>{document.hidden||D({quiet:!0})});async function _e(){let t=await Me(),e=t&&t.getPlayerName?t.getPlayerName():null;if(!e){N.innerHTML='<div class="empty-state"><b>Sign in to open the party view.</b>DM only.</div>';return}if(!(e==="DM"||t&&t.isDm&&t.isDm())){N.innerHTML='<div class="empty-state"><b>DM only.</b>Your own sheet lives at /sheet/.</div>';return}N.addEventListener("click",Ie);let n=document.getElementById("vos-party-pause");n&&n.addEventListener("click",()=>{q=!q,n.textContent=q?"Resume updates":"Pause updates",n.setAttribute("aria-pressed",String(q)),q||D()});let s=document.getElementById("vos-party-refresh");s&&s.addEventListener("click",()=>D()),await D(),Be()}N&&_e();})();
