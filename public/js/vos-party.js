(()=>{function wt(e){let t=window.VOS_PWA;return t&&t.authHeaders?t.authHeaders(e||{}):e||{}}async function St(e,t){let s=await fetch(e,{cache:"no-store",...t||{},headers:wt({"Content-Type":"application/json",...(t||{}).headers||{}})}),a=await s.json().catch(()=>({}));if(!s.ok){let i=new Error(a.error||`HTTP ${s.status}`);throw i.status=s.status,i.code=a.error_code,i}return a}function j(e,t){let s=t?{...e,playerName:t}:e;return St("/api/play/op",{method:"POST",body:JSON.stringify(s)})}function tt(e,t,s){if(!e)return e;let a=JSON.parse(JSON.stringify(e)),i=s&&s.maxHp;switch(t.op){case"damage":{let o=Math.min(a.hp.temp||0,t.amount);return a.hp.temp-=o,a.hp.current!=null&&(a.hp.current=Math.max(0,a.hp.current-(t.amount-o))),a.hp.current===0?null:a}case"heal":{a.hp.current==null&&(a.hp.current=0);let o=a.hp.current+t.amount;return a.hp.current=i!=null?Math.min(o,i):o,a}case"spendSlot":{let o=String(t.level),f=((s||{}).slots||{})[o],C=(a.slots[o]||0)+1;return f!=null&&C>f?null:(a.slots[o]=C,a)}case"restoreSlot":{let o=String(t.level);return a.slots[o]=Math.max(0,(a.slots[o]||0)-1),a}case"useCharge":{let o=(a.uses[t.feature]||0)+1;return t.max!=null&&o>t.max?null:(a.uses[t.feature]=o,a)}case"restoreCharge":return a.uses[t.feature]=Math.max(0,(a.uses[t.feature]||0)-1),a;default:return null}}function et(e,t){switch(e.op){case"damage":return{op:"setHp",value:t.hp.current,_restore:t};case"heal":return{op:"setHp",value:t.hp.current};case"setHp":return{op:"setHp",value:t.hp.current};case"setTempHp":return{op:"setTempHp",value:t.hp.temp,keepHigher:!1};case"spendSlot":return{op:"restoreSlot",level:e.level};case"restoreSlot":return{op:"spendSlot",level:e.level};case"useCharge":return{op:"restoreCharge",feature:e.feature};case"restoreCharge":return{op:"useCharge",feature:e.feature};case"addCondition":return{op:"removeCondition",condition:e.condition};case"removeCondition":return{op:"addCondition",condition:e.condition};case"adjustExhaustion":return{op:"adjustExhaustion",delta:-e.delta};case"setExhaustion":return{op:"setExhaustion",value:t.exhaustion};default:return null}}var R=new Map;async function _(e){if(R.has(e))return R.get(e);let t=fetch(e,{cache:"default"}).then(s=>{if(!s.ok)throw new Error(`HTTP ${s.status}`);return s.json()}).catch(s=>{throw R.delete(e),s});return R.set(e,t),t}function U(e){return _(e)}function st(){return _("/data/play/conditions.json")}var at={bard:"bard",cleric:"cleric",ranger:"ranger",warlock:"warlock",wizard:"wizard",rogue:"wizard",fighter:"wizard"};function nt(e){let t=e&&e.classes||[];for(let s of t){let a=String(s.identifier||s.name||"").toLowerCase();if(at[a])return at[a]}return null}async function ot(e){if(!e)return null;try{return await _(`/data/play/spells-${e}.json`)}catch{return null}}function rt(e,t){if(!e||!Array.isArray(e.prepared)||!t)return null;let s=e.prepared[t-1];return typeof s=="number"?s:null}function lt(){return U("/data/play/masquerade.json")}function it(){return U("/data/play/forms.json")}function ct(e,t){if(!e||!t)return[];let s=new Set((e.features||[]).map(a=>(a.name||"").toLowerCase()).filter(a=>a.startsWith("maschera ")).map(a=>a.split(" ")[1]));return Object.values(t.masks).filter(a=>s.has(a.key)).sort((a,i)=>a.name.localeCompare(i.name))}function W(e){return e&&e.level>=6?3:1}function z(e,t,s){return!e||!t||!t.type?[]:(e[t.type]||[]).filter(a=>a.crValue<=s)}function O(e){let t=Math.max(0,Math.floor(e/1e3));return`${Math.floor(t/60)}:${String(t%60).padStart(2,"0")}`}function ut(e,t){let s=[],a=t&&t.spellcasting&&t.spellcasting.dc,i=t&&(t.abilities||[]).find(o=>o.key==="int")||null;return i&&e.abilities&&e.abilities.int!==i.score&&s.push({label:"Intelligence",value:`${i.score} (${i.mod>=0?"+":""}${i.mod})`,why:"Yours \u2014 only Intelligence, memories and alignment stay."}),a&&s.push({label:"Save DC",value:String(a),why:"Any DC in this creature's abilities uses your spell save DC."}),s.push({label:"Bardic Inspiration",value:"kept",why:"Retained in any form. You cannot cast unless the form can."}),s}var Ct=[1,2,3,4,5,6,7,8,10,12,15,20],xt=12;function pt(e){let t=e.root,s=e.onState,a=e.playerName||null,i=e.model||null,o=e.state,f=e.limits,C=!1,x=null,E=null,H=null;function mt(){if(navigator.vibrate)try{navigator.vibrate(xt)}catch{}}async function v(c,{undoable:n=!0}={}){if(C)return;C=!0;let r=o;mt();let l=tt(o,c,f);l&&(o=l,s(o,{optimistic:!0}));try{let d=await j(c,a);o=d.state,f=d.limits||f,s(o,{note:d.note}),n&&ht(c,r,d.note)}catch(d){o=r,s(o,{error:d.message}),q(d.message,{tone:"error"})}finally{C=!1}}function ht(c,n,r){let l=et(c,n);q(r||"Done",{action:l?{label:"Undo",run:()=>v(l,{undoable:!1})}:null})}let g=null,K=null;function q(c,{tone:n="",action:r=null}={}){if(g||(g=document.createElement("div"),g.className="vos-play-toast",g.setAttribute("role","status"),document.body.appendChild(g)),g.className=`vos-play-toast is-on${n?` is-${n}`:""}`,g.innerHTML='<span class="vos-play-toast-text"></span>',g.querySelector(".vos-play-toast-text").textContent=c,r){let l=document.createElement("button");l.type="button",l.className="vos-play-toast-action",l.textContent=r.label,l.addEventListener("click",()=>{F(),r.run()}),g.appendChild(l)}clearTimeout(K),K=setTimeout(F,r?6e3:3e3)}function F(){g&&g.classList.remove("is-on")}let k=null;function L(c,n,r){b(),k=document.createElement("div"),k.className="vos-play-sheet",k.innerHTML=`
      ...
      <div class="vos-play-sheet-panel" role="dialog" aria-modal="true" aria-label="${c}">
        ...
        <div class="vos-play-sheet-body">${n}</div>
      </div>`,document.body.appendChild(k),k.addEventListener("click",d=>{d.target.closest("[data-close]")&&b()}),r&&r(k);let l=k.querySelector("button:not([data-close])");l&&l.focus()}function b(){k&&(k.remove(),k=null)}function G(){let c=f&&f.maxHp||0,n=o.hp.current!=null?o.hp.current:c,r=Ct.map(l=>`<button type="button" class="vos-play-num" data-amount="${l}">${l}</button>`).join("");L("Hit points",`
      <div class="vos-play-hp">
        <span class="vos-play-hp-now">${n}<i>/${c||"\u2014"}</i></span>
        ${o.hp.temp?`<span class="vos-play-hp-temp">+${o.hp.temp} temp</span>`:""}
      </div>
    ...
    `,c=>{c.querySelectorAll("[data-rest]").forEach(n=>{n.addEventListener("click",()=>{let r=n.dataset.rest;r==="longRest"&&!window.confirm("Take a long rest?")||(b(),v({op:r}))})})})}let vt=["blinded","charmed","deafened","frightened","grappled","incapacitated","invisible","paralyzed","petrified","poisoned","prone","restrained","stunned","unconscious"];async function Q(){let c=new Set(o.conditions||[]),n={};try{n=await st()}catch{}let r=vt.map(u=>{let y=n[u]||{};return`<div class="vos-play-cond-row${c.has(u)?" is-on":""}">
      ...
      </div>`}).join(""),l=o.concentration,d=`<div class="vos-play-conc">
    ...
    </div>`;L("Conditions",d+`<div class="vos-play-conds is-rows">${r}</div>`,u=>{u.querySelectorAll("[data-condition]").forEach(h=>{h.addEventListener("click",()=>{let $=h.dataset.condition,w=h.closest(".vos-play-cond-row"),P=!w.classList.contains("is-on");w.classList.toggle("is-on",P),v({op:P?"addCondition":"removeCondition",condition:$})})});let y=u.querySelector("[data-break]");y&&y.addEventListener("click",()=>{b(),v({op:"breakConcentration"})});let p=u.querySelector("[data-concentrate]");p&&p.addEventListener("click",()=>{let h=window.prompt("Concentrating on which spell?");b(),h&&h.trim()&&v({op:"concentrate",spell:h.trim()})})})}function m(c){return String(c??"").replace(/[&<>"]/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[n])}async function bt(){let c=i&&i.spells||[];if(!c.length){q("This character has no spells.",{tone:"error"});return}let n=await $t(),r=new Set(o.prepared||[]),l=c.map(u=>{let y=u.spells.map(p=>{let h=p.always||p.level===0,$=h||r.has(p.id),w=[p.school,...p.meta].filter(Boolean).join(" \xB7 ");return`<label class="vos-play-spell${$?" is-on":""}${h?" is-fixed":""}"
      ...
      count against the total.${n?` Your class prepares ${n} at this level; going over
      is allowed if something says so.`:""}</p>
      <div class="vos-play-spells">${l}</div>
    `,u=>{let y=u.querySelector("[data-count]");u.querySelectorAll("[data-spell]").forEach(h=>{h.addEventListener("change",()=>{h.closest(".vos-play-spell").classList.toggle("is-on",h.checked),v({op:"togglePrepared",spell:h.dataset.spell},{undoable:!1});let $=X(u);y.textContent=n?`${$} / ${n}`:String($),y.classList.toggle("is-over",!!(n&&$>n))})});let p=u.querySelector(".vos-play-search");p.addEventListener("input",()=>{let h=p.value.trim().toLowerCase();u.querySelectorAll(".vos-play-spell").forEach($=>{$.hidden=!!h&&!$.dataset.name.includes(h)})})})}function X(c){if(c)return c.querySelectorAll("[data-spell]:checked:not(:disabled)").length;let n=new Set;return(i&&i.spells||[]).forEach(r=>r.spells.forEach(l=>{(l.always||l.level===0)&&n.add(l.id)})),(o.prepared||[]).filter(r=>!n.has(r)).length}async function $t(){return x||(x=await ot(nt(i)).catch(()=>null)),rt(x,i&&i.level||0)}async function B(){return E||(E=await lt().catch(()=>null)),H||(H=await it().catch(()=>null)),!!E}async function Z(){if(!await B()){q("Could not load the masks.",{tone:"error"});return}let c=ct(i,E);if(!c.length){q("This character has no masks.",{tone:"error"});return}let n=o.mask,r=c.map(l=>`
      <button type="button" class="vos-play-mask${n&&n.key===l.key?" is-on":""}"
              data-mask="${m(l.key)}">
        <b>${m(l.name)}</b>
        <span>${m(l.type)} \xB7 ${z(H,l,W(i)).length} forms available</span>
      </button>`).join("");L("The Masquerade",`
      ${n?`<p class="vos-play-note">Wearing <b>${m(n.key)}</b> \u2014 ${O(n.remainingMs)}${n.paused?" (paused)":""} left.</p>`:""}
      ${r}
      ...
      ${n?'<button type="button" class="vos-play-secondary" data-remove>Remove mask</button>':""}
    `,l=>{l.querySelectorAll("[data-mask]").forEach(u=>{u.addEventListener("click",()=>{let y=u.dataset.mask;b();let p=(i.abilities||[]).find(w=>w.key==="cha"),h=(i.classes||[]).filter(w=>String(w.identifier).toLowerCase()==="bard").reduce((w,P)=>w+(P.levels||0),0)||i.level||0,$=p?Math.max(0,p.mod+h):0;v({op:"donMask",mask:y,tempHp:$})})});let d=l.querySelector("[data-remove]");d&&d.addEventListener("click",()=>{b(),v({op:"removeMask"})})})}async function gt(){if(!o.mask){Z();return}if(!await B())return;let c=E.masks[o.mask.key],n=W(i),r=z(H,c,n);if(!r.length){q("No forms available for this mask.",{tone:"error"});return}let l=r.map((d,u)=>`
      ...
      <p class="vos-play-note">Challenge Rating ${n} or lower${i.level<6?", rising to 3 at sixth level":""}. You keep your Intelligence and
        your spell save DC; you assume the creature's hit points.</p>
      <div class="vos-play-forms">${l}</div>
    `,d=>{d.querySelectorAll("[data-form]").forEach(u=>{u.addEventListener("click",()=>{let y=r[Number(u.dataset.form)];b(),v({op:"assumeForm",creature:y.name,source:y.source,cr:y.cr,hp:y.hp})})})})}async function kt(){if(!o.form||!await B())return"";let c=E.masks[o.mask?o.mask.key:""],r=(H&&c&&H[c.type]||[]).find(u=>u.name===o.form.creature);if(!r)return"";let l=ut(r,i).map(u=>`
      ...
      <article class="vos-play-formblock">
        ...
        <div class="vos-play-form-vitals">
          ...
          <span><b>${m(o.form.hp)}</b>/${m(o.form.maxHp)} HP</span>
          <span><b>${m(r.speed)}</b>Speed</span>
        </div>
        <div class="vos-play-overrides">${l}</div>
        ${d(r.traits,"Traits")}
        ${d(r.actions,"Actions")}
      </article>`}function T(c){let n=c.target.closest("[data-play]");if(!(!n||!t.contains(n))&&!(c.type==="keydown"&&!["Enter"," "].includes(c.key)))switch(c.type==="keydown"&&c.preventDefault(),n.dataset.play){case"hp":G();break;case"hitdice":yt();break;case"slot":{let r=Number(n.dataset.level),l=n.dataset.spent==="1";v({op:l?"restoreSlot":"spendSlot",level:r});break}case"pact":{if(n.dataset.spent==="1")return;v({op:"spendPactSlot"});break}case"charge":{let r=n.dataset.feature;if(!r)return;v({op:"useCharge",feature:r,max:Number(n.dataset.max)||void 0});break}case"exhaustion":{let r=Number(n.dataset.value);v({op:"setExhaustion",value:r===o.exhaustion?r-1:r});break}case"conditions":case"condition":Q();break;default:break}}return t.addEventListener("click",T),t.addEventListener("keydown",T),{apply:v,openHpPad:G,openRests:ft,openConditions:Q,openPrepare:bt,openMasks:Z,openForms:gt,formStatblockHtml:kt,setState(c,n){o=c,n&&(f=n)},destroy(){t.removeEventListener("click",T),t.removeEventListener("keydown",T),b(),F()}}}var Et=12e3,M=document.getElementById("vos-party-root"),J=document.getElementById("vos-party-status"),I=[],V=null,A=!1;function S(e){return String(e??"").replace(/[&<>"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[t])}function Lt(e=6e3){return new Promise(t=>{let s=Date.now();(function a(){if(window.VOS_PWA)return t(window.VOS_PWA);if(Date.now()-s>e)return t(null);setTimeout(a,80)})()})}function Ht(e){let t=window.VOS_PWA;return t&&t.authHeaders?t.authHeaders(e||{}):e||{}}async function qt(){let e=await fetch("/api/play/party",{cache:"no-store",headers:Ht()}),t=await e.json().catch(()=>({}));if(!e.ok){let s=new Error(t.error||`HTTP ${e.status}`);throw s.status=e.status,s}return t.party||[]}function At(e,t){if(!t||e==null)return"";let s=e/t;return e===0?"is-down":s<=.25?"is-bloodied":s<=.5?"is-hurt":""}function Mt(e){let t=e.limits&&e.limits.slots||{},s=e.state.slots||{},a=Object.keys(t).sort();return a.length?a.map(i=>{let o=t[i],f=Math.max(0,o-(s[i]||0));return`<span class="vos-party-slot${f?"":" is-empty"}">
      <i>${S(i)}</i>${f}<b>/${o}</b></span>`}).join(""):""}function Nt(e){let t=e.state,s=e.limits&&e.limits.maxHp||0,a=t.hp.current!=null?t.hp.current:s,i=s?Math.max(0,Math.min(100,a/s*100)):0,o=(t.conditions||[]).includes("dying"),f=(t.conditions||[]).filter(x=>x!=="dying"),C=Math.max(0,(e.limits&&e.limits.hitDice||0)-(t.hitDiceSpent||0));return e.hasStatblock?`<article class="vos-party-card ${At(a,s)}${o?" is-dying":""}"
    ...
    <button type="button" class="vos-party-hp" data-act="hp"
      ...
      ${o?'<span class="vos-party-dying">Dying</span>':""}
    </button>
